...
const f_customer_entry = z.object({ id: z.string(), name: z.string() });
const f_customer_ticket_entry = z.object({ customer_id: z.string(), body: z.string() });
export const feedback_create_customer = async (cu: typeof f_customer_entry._type) => {
    return await linearClient.createCustomer(cu)
}

export const create_customer = t.procedure.input(f_customer_entry).query(async (d) => {
    return await feedback_create_customer(d.input)
})
export const create_customer_ticket = t.procedure.input(f_customer_ticket_entry).mutation(async (d) => {
    ...
    return await linearClient.createCustomerNeed({
